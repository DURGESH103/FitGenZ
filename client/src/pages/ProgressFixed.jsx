...
export default function Progress() {
  ...
  const chartData = history.map((p, index) => ({
    id: p._id || `progress-${index}`, // Add unique ID
  ...
  // Summary cards data with unique keys
  const summaryCards = [
    { 
      key: 'current',
      label: 'Current', 
      ...
      sub: 'latest entry', 
      color: 'text-white', 
      grad: 'from-purple-500/20 to-pink-500/10 border-purple-500/25' 
    },
    { 
      key: 'starting',
      label: 'Starting', 
      value: first ? `${first.weight} kg` : '—', 
      sub: 'first entry', 
      color: 'text-slate-300', 
      grad: 'from-slate-500/15 to-slate-500/5 border-slate-500/20' 
    },
    { 
      key: 'change',
      label: 'Change', 
      ...
      sub: 'total', 
      color: diffColor, 
      grad: diff > 0 ? 'from-red-500/15 to-red-500/5 border-red-500/20' : 'from-green-500/15 to-green-500/5 border-green-500/20' 
    },
  ]
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-5 pb-24 md:pb-8">
      ...
      {latest && (
        <div className="grid grid-cols-3 gap-3">
          {summaryCards.map(({ key, label, value, sub, color, grad }) => (
            <motion.div
              key={key}
